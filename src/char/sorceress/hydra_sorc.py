...
class HydraSorc(Sorceress):
    ...
    def _fire_ball(self, cast_pos_abs: tuple[float, float], delay: tuple[float, float] = (0.16, 0.23), dist: int = 0):
                    ...
                    if monster:
                        ...
                        if time.time() - last_move > reposition_time and reposition_pos is not None:
                            ...
                            if monster and monster["dist"] <= max_distance and monster["dist"] > min_distance:
                                ...
                                if hydra_time is not None: Logger.debug(f"{time.time() - hydra_time} > 25 = {time.time() - hydra_time > 25}")
                                if hydra_distance is not None: Logger.debug(f"{math.dist(hydra_distance, monster['position_abs'])} > 60 = {math.dist(hydra_distance, monster['position_abs']) > 60}")
                                
                                if hydra_time is None or time.time() - hydra_time > 25: # or math.dist(hydra_distance, monster['position_abs']) > 60:
                                    self._hyrda(cast_pos_abs=monster['position_abs'], dist=monster['dist'])
                                    hydra_time = time.time()
                                    hydra_distance = monster['position_abs']
    ...
    def kill_mephisto(self) -> bool:
        return self._kill_super_unique("Mephisto", radius=10)

    def kill_andariel(self) -> bool:
        return self._kill_super_unique("Andariel", radius=10)

    def kill_summoner(self) -> bool:
        return self._kill_super_unique("Summoner", radius=10)

    def kill_eldritch(self) -> bool:
        return self._kill_super_unique("Eldritch", radius=10)

    def kill_shenk(self):
        return self._kill_super_unique("Shenk", radius=10)

    def kill_countess(self) -> bool:
        return self._kill_super_unique("Countess", radius=10)