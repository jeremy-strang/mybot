...
from utils.restart import kill_game, process_exists
...
def on_exit(game_controller: GameController,  release_keys=[]):
    ...
    if game_controller is not None and game_controller.obs_recorder is not None:
        ...
        try:
            if game_controller.fg_proc is not None and game_controller.fg_proc.is_alive():
                game_controller.fg_proc.terminate()
                game_controller.fg_proc.join()
        except:
            pass
    os._exit(1)
...
def main():
    ...
    print(f"show_options: {show_options}")
    ...
    print(f"dump_pickles: {dump_pickles}")
